...
/*  982 */       this.timer = null;
/*      */     }
/*      */     private void repaintLeadCell() { int i = BasicTableUI.getAdjustedLead(BasicTableUI.this.table, true); int j = BasicTableUI.getAdjustedLead(BasicTableUI.this.table, false); if (i < 0 || j < 0) return;  Rectangle rectangle = BasicTableUI.this.table.getCellRect(i, j, false); BasicTableUI.this.table.repaint(rectangle); }
/*  985 */     public void focusGained(FocusEvent param1FocusEvent) { repaintLeadCell(); } public void focusLost(FocusEvent param1FocusEvent) { repaintLeadCell(); } public void keyPressed(KeyEvent param1KeyEvent) {} public void keyReleased(KeyEvent param1KeyEvent) {} private boolean canStartDrag() { if (this.pressedRow == -1 || this.pressedCol == -1) {
/*  986 */         return false;
/*      */       }
/*      */       
...
/*      */     public void keyTyped(KeyEvent param1KeyEvent) { KeyStroke keyStroke = KeyStroke.getKeyStroke(param1KeyEvent.getKeyChar(), param1KeyEvent.getModifiers()); InputMap inputMap = BasicTableUI.this.table.getInputMap(0); if (inputMap != null && inputMap.get(keyStroke) != null) return;  inputMap = BasicTableUI.this.table.getInputMap(1); if (inputMap != null && inputMap.get(keyStroke) != null) return;  keyStroke = KeyStroke.getKeyStrokeForEvent(param1KeyEvent); if (param1KeyEvent.getKeyChar() == '\r') return;  int i = BasicTableUI.getAdjustedLead(BasicTableUI.this.table, true); int j = BasicTableUI.getAdjustedLead(BasicTableUI.this.table, false); if (i != -1 && j != -1 && !BasicTableUI.this.table.isEditing() && !BasicTableUI.this.table.editCellAt(i, j)) return;  Component component = BasicTableUI.this.table.getEditorComponent(); if (BasicTableUI.this.table.isEditing() && component != null && component instanceof JComponent) { JComponent jComponent = (JComponent)component; inputMap = jComponent.getInputMap(0); Object object = (inputMap != null) ? inputMap.get(keyStroke) : null; if (object == null) { inputMap = jComponent.getInputMap(1); object = (inputMap != null) ? inputMap.get(keyStroke) : null; }  if (object != null) { ActionMap actionMap = jComponent.getActionMap(); Action action = (actionMap != null) ? actionMap.get(object) : null; if (action != null && SwingUtilities.notifyAction(action, keyStroke, param1KeyEvent, jComponent, param1KeyEvent.getModifiers())) param1KeyEvent.consume();  }  }  }
/*      */     public void mouseClicked(MouseEvent param1MouseEvent) {}
/*      */     private void setDispatchComponent(MouseEvent param1MouseEvent) { Component component = BasicTableUI.this.table.getEditorComponent(); Point point1 = param1MouseEvent.getPoint(); Point point2 = SwingUtilities.convertPoint(BasicTableUI.this.table, point1, component); this.dispatchComponent = SwingUtilities.getDeepestComponentAt(component, point2.x, point2.y); SwingUtilities2.setSkipClickCount(this.dispatchComponent, param1MouseEvent.getClickCount() - 1); }
/* 1006 */     private boolean repostEvent(MouseEvent param1MouseEvent) { if (this.dispatchComponent == null || !BasicTableUI.this.table.isEditing()) return false;  MouseEvent mouseEvent = SwingUtilities.convertMouseEvent(BasicTableUI.this.table, param1MouseEvent, this.dispatchComponent); this.dispatchComponent.dispatchEvent(mouseEvent); return true; } private void setValueIsAdjusting(boolean param1Boolean) { BasicTableUI.this.table.getSelectionModel().setValueIsAdjusting(param1Boolean); BasicTableUI.this.table.getColumnModel().getSelectionModel().setValueIsAdjusting(param1Boolean); } public void mousePressed(MouseEvent param1MouseEvent) { if (SwingUtilities2.shouldIgnore(param1MouseEvent, BasicTableUI.this.table)) {
/*      */         return;
/*      */       }
/*      */       
...
/* Location:              D:\tools\env\Java\jdk1.8.0_211\rt.jar!\javax\swing\plaf\basic\BasicTableUI.class
