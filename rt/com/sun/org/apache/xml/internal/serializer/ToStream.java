...
/*      */     
/*  481 */     setIndentAmount(
/*  482 */         OutputPropertyUtils.getIntProperty("{http://xml.apache.org/xalan}indent-amount", format));
/*      */ 
/*      */     
/*  485 */     setIndent(
/*  486 */         OutputPropertyUtils.getBooleanProperty("indent", format));
/*      */ 
/*      */ 
/*      */     
/*  490 */     String sep = format.getProperty("{http://xml.apache.org/xalan}line-separator");
/*  491 */     if (sep != null) {
...
/*      */ 
/*      */     
/*  498 */     boolean shouldNotWriteXMLHeader = OutputPropertyUtils.getBooleanProperty("omit-xml-declaration", format);
/*      */ 
/*      */     
...
/*      */ 
/*      */       
/*  539 */       encoding = Encodings.getMimeEncoding(format
/*  540 */           .getProperty("encoding"));
...
/*      */ 
/*      */     
/*  549 */     String entitiesFileName = (String)format.get("{http://xml.apache.org/xalan}entities");
/*      */     
/*  551 */     if (null != entitiesFileName) {
/*      */ 
/*      */ 
/*      */       
/*  555 */       String method = (String)format.get("method");
/*      */       
...
/*      */ 
/*      */       
/*  597 */       encoding = Encodings.getMimeEncoding(format
/*  598 */           .getProperty("encoding"));
...
/*      */     
/*      */     }
/*  629 */     else if (encoding
/*  630 */       .equals("WINDOWS-1250") || encoding
/*  631 */       .equals("US-ASCII") || encoding
/*  632 */       .equals("ASCII")) {
/*      */       
...
/*      */         
/*  729 */         format = OutputPropertiesFactory.getDefaultMethodProperties("xml");
/*      */       } else {
/*      */         
/*  732 */         format = this.m_format;
/*  733 */       }  init(output, format, true);
/*      */     }
/*  735 */     catch (UnsupportedEncodingException unsupportedEncodingException) {}
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  962 */     if (i + 1 >= end)
/*      */     {
/*  964 */       throw new IOException(Utils.messages
/*  965 */           .createMessage("ER_INVALID_UTF16_SURROGATE", new Object[] {
/*      */               
/*  967 */               Integer.toHexString(c)
/*      */             }));
/*      */     }
/*  970 */     char high = c;
/*  971 */     char low = ch[i + 1];
/*  972 */     if (!Encodings.isLowUTF16Surrogate(low)) {
/*  973 */       throw new IOException(Utils.messages
/*  974 */           .createMessage("ER_INVALID_UTF16_SURROGATE", new Object[] {
/*      */ 
/*      */               
/*  977 */               Integer.toHexString(c) + " " + 
/*      */               
...
/*      */       }
/*      */       
/* 1249 */       boolean writeCDataBrackets = (length >= 1 && escapingNotNeeded(ch[start]));
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/* 1286 */         fireCDATAEvent(ch, old_start, length);
/*      */       }
/* 1288 */     } catch (IOException ioe) {
/*      */       
/* 1290 */       throw new SAXException(Utils.messages
/* 1291 */           .createMessage("ER_OIERROR", null), ioe);
/*      */     } 
/*      */   }
/*      */ 
/*      */ 
...
/* 1433 */       int i = start; char ch1;
/* 1434 */       for (; i < end && ((ch1 = chars[i]) == ' ' || (ch1 == '\n' && this.m_lineSepUse) || ch1 == '\r' || ch1 == '\t'); 
/*      */ 
/*      */ 
/*      */ 
/*      */         
...
/*      */ 
/*      */         
/* 1471 */         while (i < end && (ch2 = chars[i]) < '' && this.m_charInfo
/*      */           
/* 1473 */           .isTextASCIIClean(ch2))
/* 1474 */           i++; 
/* 1475 */         if (i == end) {
/*      */           break;
/*      */         }
...
/*      */ 
/*      */         
/* 1484 */         if ((isCharacterInC0orC1Range(ch) || (!isXML10 && 
/* 1485 */           isNELorLSEPCharacter(ch)) || 
/* 1486 */           !escapingNotNeeded(ch) || this.m_charInfo.isSpecialTextChar(ch)) && '"' != ch) {
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */     else {
/*      */       
/* 1586 */       startClean = accumDefaultEscape(this.m_writer, ch, i, chars, end, fromTextNode, false);
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/* 1661 */         if (i + 1 >= len)
/*      */         {
/* 1663 */           throw new IOException(Utils.messages
/* 1664 */               .createMessage("ER_INVALID_UTF16_SURROGATE", new Object[] {
/*      */                   
/* 1666 */                   Integer.toHexString(ch)
/*      */                 }));
/*      */         }
/*      */ 
/*      */ 
...
/* 1676 */           throw new IOException(Utils.messages
/* 1677 */               .createMessage("ER_INVALID_UTF16_SURROGATE", new Object[] {
/*      */ 
/*      */ 
/*      */                   
...
/*      */ 
/*      */         
/* 1702 */         if (isCharacterInC0orC1Range(ch) || ("1.1"
/* 1703 */           .equals(getVersion()) && isNELorLSEPCharacter(ch))) {
/*      */           
/* 1705 */           writer.write("&#");
/* 1706 */           writer.write(Integer.toString(ch));
/* 1707 */           writer.write(59);
/*      */         }
/* 1709 */         else if ((!escapingNotNeeded(ch) || (fromTextNode && this.m_charInfo
/* 1710 */           .isSpecialTextChar(ch)) || (!fromTextNode && this.m_charInfo
/* 1711 */           .isSpecialAttrChar(ch))) && this.m_elemContext.m_currentElemDepth > 0) {
/*      */ 
/*      */           
/* 1714 */           writer.write("&#");
...
/*      */     } 
/*      */     try {
/* 1770 */       if (true == this.m_needToOutputDocTypeDecl && null != 
/* 1771 */         getDoctypeSystem())
/*      */       {
...
/* 1848 */     startElement(elementNamespaceURI, elementLocalName, elementName, (Attributes)null);
/*      */   }
/*      */ 
/*      */   
/*      */   public void startElement(String elementName) throws SAXException {
/* 1853 */     startElement((String)null, (String)null, elementName, (Attributes)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* 2032 */           fireStartElem(this.m_elemContext.m_elementName); 
...
/* 2089 */     endElement((String)null, (String)null, name);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* 2278 */       fireCommentEvent(ch, start_old, length);
/*      */     }
/*      */   }
/*      */ 
/*      */ 
...
/* 2439 */           fireStartElem(this.m_elemContext.m_elementName); 
...
/*      */ 
/*      */       
/* 2658 */       String prefixFromRawName = ((index = rawName.indexOf(":")) < 0) ? "" : rawName.substring(0, index);
/*      */       
/* 2660 */       if (index > 0) {
/*      */ 
/*      */         
...
/*      */ 
/*      */           
/* 2731 */           addAttributeAlways("http://www.w3.org/2000/xmlns/", no_prefix ? "xmlns" : prefix, no_prefix ? "xmlns" : ("xmlns:" + prefix), "CDATA", ns, false);
/*      */         } 
/*      */       } 
/*      */     } 
/*      */   }
...
/*      */ 
/*      */           
/* 2858 */           if (existing_mapping != null && existing_mapping.m_declarationDepth == this.m_elemContext.m_currentElemDepth && 
/*      */             
...
/*      */         
/*      */         try {
/* 2899 */           String str = ensureAttributesNamespaceIsDeclared(uri, localName, rawName);
/*      */ 
/*      */ 
/*      */         
/*      */         }
...
/*      */       
/*      */       }
/* 2960 */       catch (IOException iOException) {
/*      */ 
/*      */       
/*      */       }
/* 2964 */       catch (SAXException sAXException) {}
/*      */     }
/*      */   }
/*      */ 
/*      */ 
...
/*      */         
/*      */         try {
/* 3102 */           Transformer tran = getTransformer();
/* 3103 */           if (tran != null) {
...
/* 3194 */         grow();
/*      */       }
/* 3196 */       this.m_values[++this.m_index] = val; return val;
/*      */     }
/*      */ 
/*      */ 
/*      */ 
...
/* Location:              D:\tools\env\Java\jdk1.8.0_211\rt.jar!\com\sun\org\apache\xml\internal\serializer\ToStream.class
