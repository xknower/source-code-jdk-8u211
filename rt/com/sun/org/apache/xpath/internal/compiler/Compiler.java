...
/*      */ 
/*      */     
/* 1237 */     this.m_currentPrefixResolver = null; this.m_errorHandler = errorHandler; this.m_locator = locator; this.m_functionTable = fTable; } public Expression compile(int opPos) throws TransformerException { int op = getOp(opPos); Expression expr = null; switch (op) { case 1: expr = compile(opPos + 2); return expr;case 2: expr = or(opPos); return expr;case 3: expr = and(opPos); return expr;case 4: expr = notequals(opPos); return expr;case 5: expr = equals(opPos); return expr;case 6: expr = lte(opPos); return expr;case 7: expr = lt(opPos); return expr;case 8: expr = gte(opPos); return expr;case 9: expr = gt(opPos); return expr;case 10: expr = plus(opPos); return expr;case 11: expr = minus(opPos); return expr;case 12: expr = mult(opPos); return expr;case 13: expr = div(opPos); return expr;case 14: expr = mod(opPos); return expr;case 16: expr = neg(opPos); return expr;case 17: expr = string(opPos); return expr;case 18: expr = bool(opPos); return expr;case 19: expr = number(opPos); return expr;case 20: expr = union(opPos); return expr;case 21: expr = literal(opPos); return expr;case 22: expr = variable(opPos); return expr;case 23: expr = group(opPos); return expr;case 27: expr = numberlit(opPos); return expr;case 26: expr = arg(opPos); return expr;case 24: expr = compileExtension(opPos); return expr;case 25: expr = compileFunction(opPos); return expr;case 28: expr = locationPath(opPos); return expr;case 29: expr = null; return expr;case 30: expr = matchPattern(opPos + 2); return expr;case 31: expr = locationPathPattern(opPos); return expr;case 15: error("ER_UNKNOWN_OPCODE", new Object[] { "quo" }); return expr; }  error("ER_UNKNOWN_OPCODE", new Object[] { Integer.toString(getOp(opPos)) }); return expr; } private Expression compileOperation(Operation operation, int opPos) throws TransformerException { int leftPos = getFirstChildPos(opPos); int rightPos = getNextOpPos(leftPos); operation.setLeftRight(compile(leftPos), compile(rightPos)); return operation; } private Expression compileUnary(UnaryOperation unary, int opPos) throws TransformerException { int rightPos = getFirstChildPos(opPos); unary.setRight(compile(rightPos)); return unary; } protected Expression or(int opPos) throws TransformerException { return compileOperation(new Or(), opPos); } protected Expression and(int opPos) throws TransformerException { return compileOperation(new And(), opPos); } protected Expression notequals(int opPos) throws TransformerException { return compileOperation(new NotEquals(), opPos); } protected Expression equals(int opPos) throws TransformerException { return compileOperation(new Equals(), opPos); } protected Expression lte(int opPos) throws TransformerException { return compileOperation(new Lte(), opPos); } protected Expression lt(int opPos) throws TransformerException { return compileOperation(new Lt(), opPos); } protected Expression gte(int opPos) throws TransformerException { return compileOperation(new Gte(), opPos); } protected Expression gt(int opPos) throws TransformerException { return compileOperation(new Gt(), opPos); } protected Expression plus(int opPos) throws TransformerException { return compileOperation(new Plus(), opPos); } public Compiler() { this.locPathDepth = -1; this.m_currentPrefixResolver = null; this.m_errorHandler = null; this.m_locator = null; }
...
/*      */   protected Expression number(int opPos) throws TransformerException { return compileUnary(new Number(), opPos); }
/* 1246 */   protected Expression literal(int opPos) { opPos = getFirstChildPos(opPos); return (XString)getTokenQueue().elementAt(getOp(opPos)); } protected Expression numberlit(int opPos) { opPos = getFirstChildPos(opPos); return (XNumber)getTokenQueue().elementAt(getOp(opPos)); } protected Expression variable(int opPos) throws TransformerException { Variable var = new Variable(); opPos = getFirstChildPos(opPos); int nsPos = getOp(opPos); String namespace = (-2 == nsPos) ? null : (String)getTokenQueue().elementAt(nsPos); String localname = (String)getTokenQueue().elementAt(getOp(opPos + 1)); QName qname = new QName(namespace, localname); var.setQName(qname); return var; } protected Expression group(int opPos) throws TransformerException { return compile(opPos + 2); } public PrefixResolver getNamespaceContext() { return this.m_currentPrefixResolver; } protected Expression arg(int opPos) throws TransformerException { return compile(opPos + 2); } protected Expression union(int opPos) throws TransformerException { this.locPathDepth++; try { return UnionPathIterator.createUnionIterator(this, opPos); } finally { this.locPathDepth--; }  } public int getLocationPathDepth() { return this.locPathDepth; }
...
/*      */   public int countPredicates(int opPos) throws TransformerException { int count = 0; while (29 == getOp(opPos)) { count++; opPos = getNextOpPos(opPos); }  return count; }
/* 1256 */   public void setNamespaceContext(PrefixResolver pr) { this.m_currentPrefixResolver = pr; }
/*      */ 
/*      */   
...
/* Location:              D:\tools\env\Java\jdk1.8.0_211\rt.jar!\com\sun\org\apache\xpath\internal\compiler\Compiler.class
