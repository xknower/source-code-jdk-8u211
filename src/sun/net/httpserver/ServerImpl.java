...
/*     */     
/*     */     Dispatcher() {
/* 310 */       this.connsToRegister = new LinkedList<>();
/*     */     } private void handleEvent(Event param1Event) { ExchangeImpl exchangeImpl = param1Event.exchange; HttpConnection httpConnection = exchangeImpl.getConnection(); try { if (param1Event instanceof WriteFinishedEvent) { int i = ServerImpl.this.endExchange(); if (ServerImpl.this.terminating && i == 0)
/*     */             ServerImpl.this.finished = true;  ServerImpl.this.responseCompleted(httpConnection); LeftOverInputStream leftOverInputStream = exchangeImpl.getOriginalInputStream(); if (!leftOverInputStream.isEOF())
/*     */             exchangeImpl.close = true;  if (exchangeImpl.close || ServerImpl.this.idleConnections.size() >= ServerImpl.MAX_IDLE_CONNECTIONS) { httpConnection.close(); ServerImpl.this.allConnections.remove(httpConnection); } else if (leftOverInputStream.isDataBuffered()) { ServerImpl.this.requestStarted(httpConnection); handle(httpConnection.getChannel(), httpConnection); } else { this.connsToRegister.add(httpConnection); }  }
/*     */          }
/*     */       catch (IOException iOException) { ServerImpl.this.logger.log(Level.FINER, "Dispatcher (1)", iOException); httpConnection.close(); }
/* 316 */        } void reRegister(HttpConnection param1HttpConnection) { try { SocketChannel socketChannel = param1HttpConnection.getChannel();
