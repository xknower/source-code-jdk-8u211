...
/*     */     } 
/*     */   }
/*     */   synchronized void add(Object paramObject, DisposerRecord paramDisposerRecord) { WeakReference weakReference; if (paramObject instanceof DisposerTarget)
/*     */       paramObject = ((DisposerTarget)paramObject).getDisposerReferent();  if (refType == 1) { PhantomReference phantomReference = new PhantomReference(paramObject, queue); }
/*     */     else
/*     */     { weakReference = new WeakReference(paramObject, queue); }
/*     */      records.put(weakReference, paramDisposerRecord); } public void run() { while (true) {
/*     */       try {
/*     */         while (true) {
/*     */           Reference reference = queue.remove(); reference.clear(); DisposerRecord disposerRecord = (DisposerRecord)records.remove(reference); disposerRecord.dispose(); reference = null; disposerRecord = null; clearDeferredRecords();
/*     */         }  break;
...
/* 253 */     records.put(new WeakReference(paramObject, queue), paramDisposerRecord); } private static void clearDeferredRecords() { if (deferredRecords == null || deferredRecords.isEmpty())
/*     */       return;  for (byte b = 0; b < deferredRecords.size(); b++) { try { DisposerRecord disposerRecord = deferredRecords.get(b); disposerRecord.dispose(); }
/*     */       catch (Exception exception)
/*     */       { System.out.println("Exception while disposing deferred rec."); }
/*     */        }
/*     */      deferredRecords.clear(); }
/* 259 */   public static ReferenceQueue getQueue() { return queue; }
/*     */ 
/*     */   
...
/* Location:              D:\tools\env\Java\jdk1.8.0_211\rt.jar!\sun\java2d\Disposer.class
